...
아래 여섯 가지는 모델 출력이 흔들리는 축이라 특히 조용히 깨지기 쉽다.
  ① 해설을 선지별로 ①②③④⑤ 로 나열
  ② '선택지:' 머리말 생략
  ③ 객관식이 아닌데 해설이 번호로 시작
  ④ 선택지 기호를 ①②③④⑤ 가 아닌 a) · 1. · 가) 로 씀
  ⑤ 같은 선택지 목록을 두 번 뱉음
...
# ──────────────────────────────────────────────
# ⑤ 선택지를 ①②③④⑤ 가 아닌 'a) b) c) d)' 로 쓴 경우.
#    실제로 이렇게 생성된 세트가 있었다. 이 표기를 못 알아보면 선택지가 리스트가
#    아니라 문자열 한 덩어리가 되고, 화면은 Array.isArray 로 걸러내 **객관식인데
#    보기가 한 줄도 안 나오는** 카드가 된다. 정답도 'c) 본문…' 형태라 몇 번인지
#    알 수 없어 정답 표시까지 빠진다 — 같은 원인의 두 증상.
# ──────────────────────────────────────────────

ALPHA_MARKS = """유형: 객관식
번호: 1
문제: 프리온 단백질에 대한 설명으로 옳은 것은?

선택지:
a) 정상 프리온(PrPC)은 β-병풍 구조가 주를 이룬다.
b) 변형될 때 아미노산 서열(1차 구조)이 바뀐다.
c) 감염형 프리온은 정상 프리온의 구조 변형을 유도해 자가전파된다.
d) 감염성은 프리온에 포함된 RNA 서열이 매개한다.

정답: c) 감염형 프리온은 정상 프리온의 구조 변형을 유도해 자가전파된다.
해설: 1차 구조는 그대로이고 3차 구조만 바뀝니다.
함정포인트: 서열이 바뀐다고 착각하기 쉽습니다."""

print("\n⑤ 선택지를 'a) b) c) d)' 로 쓴 경우")
q5 = parse_question_block(ALPHA_MARKS)
c5 = q5.get("선택지") or []
check("선택지가 리스트", isinstance(c5, list), type(c5).__name__)
check("선택지 4개", len(c5) == 4, f"{len(c5)}개")
check("기호를 ①②③④ 로 통일", [c[:1] for c in c5] == ["①", "②", "③", "④"],
      [c[:1] for c in c5])
check("선택지 본문 유지", "자가전파" in (c5[2] if len(c5) > 2 else ""),
      repr(c5[2] if len(c5) > 2 else "")[:60])
check("정답 기호를 ③ 으로 옮김", (q5.get("정답") or "").startswith("③"), q5.get("정답"))
check("해설이 선택지로 새지 않음", "3차 구조" in (q5.get("해설") or ""),
      repr(q5.get("해설"))[:60])
check("함정포인트 유지", "착각" in (q5.get("함정포인트") or ""))

# 줄바꿈된 선택지는 앞 선택지에 이어 붙어야 한다 (새 선택지로 세면 개수가 틀어진다)
WRAPPED = """유형: 객관식
문제: 다음 중 옳은 것은?
선택지:
a) 첫 번째 선택지인데 길어서
...
q6 = parse_question_block(WRAPPED)
c6 = q6.get("선택지") or []
check("줄바꿈 선택지를 2개로 셈", len(c6) == 2, f"{len(c6)}개")
check("이어진 줄이 앞 선택지에 붙음", "다음 줄로" in (c6[0] if c6 else ""),
      repr(c6[0] if c6 else "")[:60])

# ──────────────────────────────────────────────
# ⑥ 모델이 같은 선택지 목록을 두 번 뱉는 경우 — 화면에 선지가 10개로 나왔다.
#    실제 형태(배포 generation 8): 정답을 '①, ③ (복수정답)' 으로 쓴 뒤 스스로
#    의심했는지 '선택지:' 블록을 통째로 다시 출력하고 '정답: ①' 로 고쳐 썼다.
#    ⑤의 관대함('선택지:' 아래 줄은 기호가 뭐든 받는다)을 넣을 때 여기에
#    in_answer 가드를 안 걸어서 두 번째 목록까지 주워 담았다.
# ──────────────────────────────────────────────
...
# ⑥-1 정답을 쓴 뒤 선택지 블록이 통째로 다시 나오는 경우
REPEAT_AFTER_ANSWER = f"""유형: 객관식
문제: 다음 중 옳은 것은?
선택지:
{CHOICES}
정답: ①, ③ (복수정답)
...
# ⑥-2 같은 '선택지:' 블록 안에서 두 번 나열하는 경우
REPEAT_IN_BLOCK = f"""유형: 객관식
문제: 다음 중 옳은 것은?
선택지:
{CHOICES}
{CHOICES}
...
print("\n⑥ 선택지 목록이 두 번 나오는 경우")
for name, block, want_answer in (("정답 뒤 재출력", REPEAT_AFTER_ANSWER, "③"),
                                 ("블록 안 중복 나열", REPEAT_IN_BLOCK, "③")):
    qd = parse_question_block(block)
    cd = qd.get("선택지") or []
    check(f"{name}: 선택지 5개", len(cd) == 5, f"{len(cd)}개")
    check(f"{name}: 정답 유지", qd.get("정답") == want_answer, qd.get("정답"))
    check(f"{name}: 해설 유지", "다가 옳다" in (qd.get("해설") or ""),
          repr(qd.get("해설"))[:50])
