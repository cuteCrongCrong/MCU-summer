...
#
#   200 → 500 (2026-08). 디스크 기준으로는 훨씬 더 올려도 되지만 그 위는 의미가 없다 —
#   프롬프트에 들어가는 글자가 한쪽당 70만 자에서 잘리므로(llm.GEN_SIDE_CHAR_BUDGET)
#   텍스트 PDF는 250~350쪽이면 예산을 다 쓴다. 500MB가 필요한 경우는 스캔본뿐이다
#   (쪽당 2~3MB × 300쪽 = 600~900MB).
...
#      실측: e2-micro(RAM 1GB)가 이 기본값 그대로 300/200/12(스레드 4)로 운영 중이고
#      생성이 끊긴 적이 없다.
#   그래서 남는 제약은 셋이다:
...
#              GCP 무료 티어의 월 1GB egress가 여기서 닳는다 — 배포-GCP.md '무료 조건'.
#              그림을 흑백으로 굽기 시작하면서(llm.IMAGE_TRANSCRIBE/IMAGE_DESCRIBE의
#              gray) 장당 전송량이 1/3~1/5로 줄었다: 벡터 슬라이드 332KB → 57KB.
#     요금   — 한 쪽이 곧 Vision 호출 한 번. (같은 PDF 재분석은 image_desc_cache로 0회)
#   (흑백 A4 PNG ≈ 벡터 슬라이드 40~60KB · 스캔 페이지 30~50KB, base64 전 기준)
#
...
#   '900초 벽'은 더 이상 여기 걸리지 않는다. waitress의 channel_timeout은 총 소요가
#   아니라 **무통신** 시간이고(waitress 원문: "Maximum seconds to leave an inactive
#   connection open"), 스트리밍 경로는 그림·문제마다 이벤트를 흘려보낸다. 침묵하던
#   유일한 구간이 렌더였는데 거기도 이제 heartbeat가 나간다(features/question_gen.py).
#   그래서 상한을 올릴 때 볼 것은 타임아웃이 아니라 **대기 시간과 요금**이다.
...
#   상한을 100/60 → 300/200 으로 올리면서 8 → 12 로 맞췄다 (500쪽 ÷ 8 = 63라운드 →
#   ÷12 = 42라운드). 이제는 타임아웃이 아니라 사용자 대기 시간이 기준이다.
