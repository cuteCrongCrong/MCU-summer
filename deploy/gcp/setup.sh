...
# 30 → 50: llm.shrink_pdf_bytes가 업로드 직후(무거운 파이프라인 전) 이미지를 렌더
# 필요 해상도까지만 줄여서, 강의록을 여러 개 합쳐 올려도 30MB에 자주 걸리는 문제를
# 완화했다. 다만 상한 자체는 "줄이기 전 원본"이 통과해야 하는 크기라 여전히 낮게 둔다.
DEF_MAX_UPLOAD_MB=50
