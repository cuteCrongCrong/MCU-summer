...
## ✅ 미국 IP에서 LLM 게이트웨이 — 확인됨

이 앱의 **기본** 프로바이더는 전북대 LLM 게이트웨이(`factchat-cloud.mindlogic.ai`)입니다.
**GCP 무료 티어는 미국 리전뿐**이라, 게이트웨이가 국내 IP만 허용했다면
이 가이드 전체가 무의미해질 뻔했습니다.

> **2026-07-28 확인: 미국 VPN에서 문제 생성이 정상 동작합니다.**
> 즉 미국 리전 무료 티어를 그대로 쓸 수 있습니다. 이 가이드를 순서대로 따라가세요.

학교 정책은 언제든 바뀔 수 있으니, 한참 뒤에 이 문서를 보고 계신다면 다시 확인해보세요 —
미국 서버로 VPN을 켠 상태에서 로컬(`실행.bat`)로 문제 생성을 한 번 돌려보면 5분이면 됩니다.
(설치 스크립트도 VM에서 마지막 단계에 이 연결을 자동 점검합니다)

### 나중에 막히더라도 배포가 무의미해지지는 않습니다

이 앱은 **OpenAI · Anthropic · Gemini** 도 지원합니다([providers/factory.py](providers/factory.py)).
그리고 **API 키는 서버에 저장되지 않고 사용자가 화면에서 요청마다 입력**합니다
([features/question_gen.py:362](features/question_gen.py:362)).

즉 게이트웨이가 미국 IP를 막아도, 각자 자기 API 키로 다른 제공사를 고르면 그대로 씁니다.
**대신 그 제공사는 유료**입니다 — 게이트웨이는 학교가 대주는 것이니까요.

| 상황 | 선택지 |
|---|---|
| **게이트웨이가 미국에서 열림** ← **현재 상태** | 그대로 무료 배포 (가장 좋음) |
| 막힘 + 각자 API 키를 쓸 수 있음 | 그대로 무료 배포, LLM 비용만 개인 부담 |
| 막힘 + 게이트웨이를 꼭 써야 함 | [서울 리전으로](#게이트웨이가-막힐-때--서울-리전으로) (월 $18) |
...
미국 IP가 차단됐고, **다른 제공사(OpenAI 등)로 우회하는 것도 원치 않는다면** —
즉 전북대 게이트웨이를 꼭 써야 한다면 — 무료 티어를 포기하고 서울 리전으로 갑니다.

> **2026-07-28 기준으로는 이 절이 필요 없습니다.** 미국 IP에서 게이트웨이가
> 정상 동작하는 것을 확인했습니다(문서 맨 위 참고). 나중에 정책이 바뀌어
> 막히게 됐을 때를 위해 남겨둡니다.
