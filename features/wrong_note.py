...
사용자별 분리: 폴더는 current_owner()로 소유자 필터. 담긴 문제(items)는
             소유한 폴더를 통해서만 접근.
             게스트도 브라우저별 익명 id(guest_id)로 격리되어 서로의 폴더를 못 본다.
...
def create_wrong_folder(name: str, owner) -> int:
    user_id, guest_id = owner
    conn = get_conn()
    try:
        cur = conn.execute(
            "INSERT INTO wrong_folders (name, created_at, user_id, guest_id) VALUES (?,?,?,?)",
            (name, datetime.now().strftime("%Y-%m-%d %H:%M"), user_id, guest_id),
        )
        conn.commit()
...
def list_wrong_folders(owner) -> list:
    """오답 폴더 목록 + 각 폴더의 문제 개수. 소유자 것만, 최신순."""
    frag, params = owner_clause(owner, "f.")
    conn = get_conn()
    try:
        rows = conn.execute(
...
def load_wrong_folder(fid: int, owner):
...
def rename_wrong_folder(fid: int, name: str, owner):
    frag, params = owner_clause(owner)
    conn = get_conn()
    try:
        conn.execute(
...
def delete_wrong_folder(fid: int, owner):
...
def add_wrong_item(fid: int, question: dict, owner) -> dict:
...
def delete_wrong_item(item_id: int, owner):
    """소유한 폴더의 문제만 삭제."""
    frag, params = owner_clause(owner)
    conn = get_conn()
    try:
...
# ──────────────────────────────────────────────
# 라우트 (current_owner()로 소유자 분리)
# ──────────────────────────────────────────────

@wrong_bp.route("/wrong-folders", methods=["GET"])
def wrong_folders_list():
    return jsonify({"folders": list_wrong_folders(current_owner())})
...
def wrong_item_add(fid):
    ...
    result = add_wrong_item(fid, question, current_owner())
    if result.get("error"):
...
def wrong_item_delete(item_id):
    delete_wrong_item(item_id, current_owner())
    return jsonify({"success": True})
