...
class Provider(ABC):
    ...
    # ── 탭별 기본 선택 모델 (위 셋과 성격이 다르다 — 강제가 아니라 '기본값') ──
    # 사용자가 드롭다운에서 얼마든지 바꿀 수 있고, 서버는 그 선택을 그대로 쓴다.
    # 화면이 첫 선택으로 삼으려면 알아야 하는 값이라 /providers 로 내려보낸다
    # (위 셋은 서버만 쓰므로 안 내려보낸다 — providers/factory.py 주석 참고).

    # 기출 주제 분석 탭의 기본 선택 모델. 빈 값이면 default_model 을 쓴다.
    #   탭마다 갈라 두는 이유: 주제 분석은 강의록·기출을 **한 프롬프트에 합쳐서**
    #   대조표를 만들기 때문에(llm.TOPIC_SIDE_CHAR_BUDGET), 사용자가 고른 모델 하나가
    #   14만 토큰을 통째로 읽는다. 문제 생성 탭에서 같은 모델이 받는 양(실측
    #   generation 209: 5,672토큰)의 25배라, 두 탭이 기본값을 공유하면 주제 분석
    #   쪽만 그 기본값의 단가를 14만 토큰에 그대로 곱하게 된다.
    topic_default_model = ""
