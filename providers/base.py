...
#    그림 문제(부위 이름 쓰기 등)를 살리려면 그림의 의미가 텍스트로 남아야 한다.
#    손글씨는 버리지 않고 (필기: …)로 '표시해서' 남긴다 — 인쇄된 문제와 섞이면 학생이
#    덧쓴 정답 동그라미가 시험지에 원래 있던 내용으로 굳고(build_exam_analysis_prompt의
#    '정답' 규칙), 통째로 버리면 손으로 정리한 기출 복원본이 사라진다.
#    (판독불가)도 같은 이유 — 흐린 손글씨를 그럴듯하게 지어내면 티가 안 난다.
IMAGE_DESC_PROMPT = (
...
class Provider(ABC):
    ...
    # ── 모델 고정 필드 (아래 셋은 모두 '빈 값이면 호출부가 정한 모델을 쓴다') ──
    # 여러 제공사 모델을 한 키로 부르는 게이트웨이에서만 의미가 있어 대부분 빈 값이다.
    # 모델 id는 프로바이더 안에서만 유효하므로 llm.py가 아니라 여기에 둔다 —
    # 게이트웨이용 id를 llm.py에 박으면 다른 키를 쓰는 사용자에겐 없는 모델이 된다.

    # 이미지 호출에만 강제로 쓸 모델. '읽고 옮기기'라 본작업과 체급을 맞출 이유가 없다.
    #   ⚠️ 쓸지 말지는 **기능이 정한다.** 지금 참조하는 곳은 기출 주제 분석뿐 —
    #      거기서는 이미지 호출만 이 모델로 가고 주제↔문항 대조는 사용자가 고른 모델이 한다.
    image_model = ""

    # 분석 단계 **전체**에 강제로 쓸 모델 (문제 생성기가 본다).
    #   image_model과 값이 같아도 뜻이 다르다 — 이쪽은 그림 읽기뿐 아니라 개념·형식
    #   분석까지 포함한다. "그림은 싼 모델, 텍스트 분석은 중간 모델"로 가르고 싶어질 때가
    #   있으므로 하나로 합치지 말 것.
    #   고정하는 이유: 이 단계에서 입력 토큰의 90%가 나간다(실측 generation 92 —
    #   전체 입력 119,521토큰 중 103,392). 매번 고르게 두면 비싼 모델이 그 90%에 붙는다.
    analysis_model = ""

    # 이미지 호출이 실패했을 때 한 번 더 시도할 모델. 빈 값이면 폴백 없이 실패한다.
    image_fallback_model = ""

    # 아래 두 메서드가 공통으로 받는 인자:
    #   usage        — providers.usage.UsageCollector. 넘기면 구현체가 호출마다 토큰을
    #                  기록한다. None이면 아무것도 하지 않는다.
    #   cache_prefix — prompt 앞에 붙는 '여러 요청에서 똑같이 반복되는 부분'. 프롬프트
    #                  캐싱을 지원하는 제공사(Anthropic)는 여기에 캐시 경계를 잡고,
    #                  아닌 곳은 그냥 이어붙인다 → 호출부는 제공사를 몰라도 된다.
