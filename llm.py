...
"""
    ...
    # ── 증례 지시 모순 해소 ──
    # [2] 형식 키워드가 '증례제시: 해당없음'인데 출력 템플릿이 "[증례 포함 문제 전체]"를
    # 못박으면 같은 프롬프트가 서로 반대를 지시한다. 증례가 없으면 템플릿에서 뺀다.
    no_vignette = any(
...
#   총 60만 자 × 0.52토큰/자 ≈ 31만 토큰. 출력 몫(TOPIC_MAX_TOKENS)과 지시문을 더해도
#   1M 컨텍스트의 1/3이다. 이미지 상한(강의록 전사 100쪽 ≈ 8만 자)을 받아줄 수 있어야
