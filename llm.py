...
# 업로드 용량 상한(config.MAX_UPLOAD_MB)에 걸리는 큰 PDF를 서버가 자동으로 줄인다.
#   대상: 이 크기 미만이면 손대지 않는다 (작은 파일을 매번 열어보는 비용이 손해).
SHRINK_MIN_SIZE_MB = 5.0
# 이미지를 얼마나 줄여도 되는지의 기준선. TRANSCRIBE_RENDER_DPI(강의록 전사, 150)를
# 쓰는 이유는 두 렌더 모드(전사 150 / 설명 110) 중 더 높은 쪽이라, 여기 맞추면
# 어느 모드로 나중에 렌더링해도 해상도가 모자라지 않는다.
SHRINK_TARGET_DPI = TRANSCRIBE_RENDER_DPI
# 목표 해상도에 여유를 더 준다 — bbox 계산이나 렌더링의 미세한 오차로 딱 맞춰 깎으면
# 나중에 오히려 살짝 못 미칠 수 있어서다.
SHRINK_HEADROOM = 1.15
SHRINK_JPG_QUALITY = 78


def shrink_pdf_bytes(pdf_bytes: bytes) -> bytes:
    """
    PDF 안에 박힌 그림만 줄여서 파일 용량을 줄인다. 텍스트 레이어·페이지 구성·
    벡터 그래픽은 그대로 둔다 — read_pdf_pages의 판별 로직(텍스트 길이·이미지
    면적비·벡터 필기)이 전부 이 정보에 기대므로, 손대면 판별 자체가 달라진다.

    이미지 하나를 얼마나 줄여도 되는지는 "이 이미지가 페이지에서 실제로 차지하는
    크기(bbox, pt) × SHRINK_TARGET_DPI"로 정한다 — 그 이상 해상도는 read_pdf_pages가
    렌더링할 때(get_pixmap(dpi=...)) 애초에 버려지는 초과분이라 압축해도 화질
    손해가 없다. 네이티브 해상도가 이미 그 기준보다 작으면 건드리지 않는다
    (확대는 화질을 더 나쁘게 만들 뿐이다).

    실패해도(암호화·손상 등) 예외를 던지지 않고 원본을 그대로 반환한다 — 압축은
    최적화일 뿐이라, 여기서 막히면 업로드 자체가 막혀서는 안 된다.
    """
    if len(pdf_bytes) < SHRINK_MIN_SIZE_MB * 1024 * 1024:
        return pdf_bytes
    try:
        ...
        return pdf_bytes
    try:
        # xref(이미지 객체 하나) → 그 이미지가 쓰이는 모든 페이지 중 가장 큰 필요 해상도
        needed_px = {}
        for page in doc:
            for info in page.get_image_info(xrefs=True):
                ...
                need_w = bw / 72.0 * SHRINK_TARGET_DPI * SHRINK_HEADROOM
                need_h = bh / 72.0 * SHRINK_TARGET_DPI * SHRINK_HEADROOM
                cur = needed_px.get(xref, (0.0, 0.0))
                needed_px[xref] = (max(cur[0], need_w), max(cur[1], need_h))
        if not needed_px:
            doc.close()
            return pdf_bytes
        ...
        for page in doc:
            for img in page.get_images(full=True):
                xref = img[0]
                if xref in seen or xref not in needed_px:
                    continue
                seen.add(xref)
                need_w, need_h = needed_px[xref]
                try:
                    pix = fitz.Pixmap(doc, xref)
                    if pix.alpha:
                        pix = fitz.Pixmap(pix, 0)               # 알파 제거 (JPEG는 알파 없음)
                    if pix.colorspace is None or pix.colorspace.n not in (1, 3):
                        pix = fitz.Pixmap(fitz.csRGB, pix)      # CMYK 등 → RGB
                    w, h = pix.width, pix.height
                    if w > need_w and h > need_h:
                        ratio = min(need_w / w, need_h / h)
                        pix = fitz.Pixmap(pix, max(1, int(w * ratio)),
                                          max(1, int(h * ratio)), None)
                    new_bytes = pix.tobytes("jpeg", jpg_quality=SHRINK_JPG_QUALITY)
                    for p2 in doc:
                        for img2 in p2.get_images(full=True):
                            if img2[0] == xref:
                                p2.replace_image(xref, stream=new_bytes)
                                changed = True
                                break
                except Exception:
                    continue   # 이 이미지 하나만 실패 — 나머지는 계속 처리한다
